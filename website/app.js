...
/* voice waveform: stagger each bar so the ripple travels across the row */
$$('.wave i').forEach((b, i) => b.style.animationDelay = (-i * 0.08) + 's');
...
(function dialDemo() {
  ...
  let open = false, path = [], hintLock = 0, cx = 0, cy = 0;
  const TAU = Math.PI * 2;
  ...
  function render(bornLvl = -1) {
    const base = Math.min(wrap.clientWidth, wrap.clientHeight) || 480;
    cx = (wrap.clientWidth || base) / 2;
    cy = (wrap.clientHeight || base) / 2;
    svg.setAttribute('viewBox', `0 0 ${wrap.clientWidth || base} ${wrap.clientHeight || base}`);
    svg.textContent = '';
    bud.classList.toggle('open', open);
    if (!open) return;

    const ns = base * 0.11, pad = base * 0.03, gapPx = base * 0.006;
    const levels = [TREE.children, ...path.map(p => p.children || [])];
    let prevOut = 0, parentAng = -Math.PI / 2;

    levels.forEach((kids, lvl) => {
      if (!kids.length) return;
      const rMid = lvl === 0 ? base * 0.29 : prevOut + base * 0.02 + ns / 2 + pad;
      const rIn = rMid - ns / 2 - pad, rOut = rMid + ns / 2 + pad;
      prevOut = rOut;
      ...
      if (lvl === 0) {
        ...
        half = step / 2;
        angles = kids.map((_, i) => parentAng - total / 2 + step * (i + 0.5));
      }

      const ring = el('g', 'dial-ring');
      const dIn = Math.asin(Math.min(.5, gapPx / rIn)), dOut = Math.asin(Math.min(.5, gapPx / rOut));
      const trail = path[lvl];

      // spoke connecting the parent wedge to this ring
      if (lvl > 0) {
        const sp = el('line', 'dial-spoke');
        const a = parentAng, pIn = { x: cx + (rIn - 4) * Math.cos(a), y: cy + (rIn - 4) * Math.sin(a) };
        sp.setAttribute('x1', cx + (rMid - ns) * Math.cos(a)); sp.setAttribute('y1', cy + (rMid - ns) * Math.sin(a));
        sp.setAttribute('x2', pIn.x); sp.setAttribute('y2', pIn.y);
        ring.appendChild(sp);
      }
      ...
      kids.forEach((node, i) => {
        ...
        if (node.children) {
          ...
          const rSub = rOut - base * 0.014, dSub = Math.asin(Math.min(.5, (gapPx + 3) / rSub));
        ...
        if (isTrail) parentAng = a;

        wg.addEventListener('pointerenter', () => say(node.l + (node.children ? ' — folder' : ''), true));
  ...
  // idle nudge: bloom itself once when it first scrolls into view
  if (!reduce) {
    const io = new IntersectionObserver((es) => {
      es.forEach(en => {
        if (!en.isIntersecting) return;
        io.disconnect();
        setTimeout(() => { if (!open) { open = true; render(0); say('Open a folder — the ring behind it dims'); } }, 900);
      });
    }, { threshold: .5 });
    io.observe(wrap);
  }
